/*
DROP TRIGGER hair_kind_no_TRI;
DROP TRIGGER HAIR_NO_TRI;
DROP TRIGGER DESIGNER_NO_TRI;
DROP TRIGGER EVENT_NO_TRI;
DROP TRIGGER BOOK_NO_TRI;
DROP TRIGGER QNA_NO_TRI;
DROP TRIGGER hboard_NO_TRI;
DROP TRIGGER coupon_id_tri;
DROP TRIGGER notice_id_tri;
DROP TRIGGER od_no_tri;
DROP TRIGGER BOOKING_HAIR_HAIRNO_TRI;
*/

DROP SEQUENCE hair_kind_no_SEQ;
DROP SEQUENCE HAIR_NO_SEQ;
DROP SEQUENCE DESIGNER_NO_SEQ;
...
/* 디자이너 번호 */
CREATE SEQUENCE DESIGNER_NO_SEQ
	START WITH 1
	INCREMENT BY 1
	NOCACHE;

CREATE OR REPLACE TRIGGER DESIGNER_NO_TRI
BEFORE INSERT ON DESIGNER
FOR EACH ROW 
BEGIN 
	IF Inserting AND :NEW.DE_NO IS NULL THEN 
		SELECT DESIGNER_NO_SEQ.NEXTVAL INTO :NEW.DE_NO FROM DUAL;
	END IF;
END; 
...
CREATE OR REPLACE TRIGGER HBOARD_NO_TRI
...
CREATE OR REPLACE TRIGGER NOTICE_ID_TRI
...
CREATE OR REPLACE TRIGGER OD_NO_TRI
