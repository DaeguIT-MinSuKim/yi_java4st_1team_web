...
--미답변한 qna 갯수
SELECT COUNT(*) AS count FROM QNA WHERE RES_YN = 'n' AND ADMIN_ID IS NULL OR NOTICE_YN ='n';
--미답변한 qna
SELECT * FROM (SELECT rownum RN, a.* FROM (SELECT * FROM QNA WHERE RES_YN = 'n' AND NOTICE_YN ='n'  ORDER BY notice_yn DESC,QNA_NO DESC ) a) WHERE QNA_REFNO IS NULL AND rn BETWEEN 1 AND 10;
...
UPDATE QNA  set DEL_YN = 'n'  WHERE QNA_NO = 1;
