...
SELECT * FROM (SELECT rownum RN, a.QNA_NO,GUEST_ID,GUEST_NAME,ADMIN_ID,QNA_TITLE,QNA_CONTENT,QNA_FILE,TO_DATE(QNA_REGDATE,'yyyy-MM-dd') as QNA_REGDATE,RES_YN,QNA_REFNO,NOTICE_YN,DEL_YN,QNA_SECRET,QNA_PASSWORD FROM (SELECT * FROM QNA WHERE DEL_YN = 'n' AND QNA_REFNO is NULL ORDER BY notice_yn DESC,QNA_NO DESC ) a) WHERE rn BETWEEN 1 AND 5; 
