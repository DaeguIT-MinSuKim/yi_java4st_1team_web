 ...
 * 예약 등록 핸들러
 * 
 * @author jisu25 BookingRegisterFormHandler -> 작성 후 submit ->
 *         BookingRegisterHandler
 */
		...
		if (request.getMethod().equalsIgnoreCase("GET")) {
			...
			if (newBooking.getBookDate().toLocalDate().isAfter(LocalDate.now())) {
				if (bService.isAvailableTime(newBooking.getBookDateStr(), deNo) != 1) {
					...
					try {
						BookingRegisterSendEmail emailThr = new BookingRegisterSendEmail();
						emailThr.setGuest(guest);
						emailThr.setInsBooking(insBooking);
