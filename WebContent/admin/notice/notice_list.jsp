	...
	<div class="card-header py-2">
		<h6 class="m-1 font-weight-bold text-primary"
			...
			<input type="button" value="등록" class="btn btn-success btn-sm"
				...
				onclick="location.href='noticeInsert.do' "> <input
				type="button" value="삭제" name="delete" class="btn btn-danger btn-sm"
				id="btn_delete" style="float: left;" onclick="checkDelete()">
					...
					<tbody>
						<c:forEach var="notice" items="${viewAll}" varStatus="status">
							<tr>
								...
								<td style="width: 100px;"><a href="noticeDetail.do?no=${notice.noticeNo }">${notice.noticeTitle}</a></td>
								<td><input type="hidden" name="hidden"
									value="${guest.guestId}"> <input type="button"
									name="info" value="삭제"
									class="btn bg-gray-200 btn-sm detailViewButton"
									onclick="buttonDel()">
									<input type="button" name="info" value="수정"
									class="btn bg-gray-200 btn-sm detailViewButton"
